 ...
 */
 ...
 public class GrammarBasedParserTest {
   ...
   @Test
    public void testAggregateStatsDirectiveParses() throws Exception {
      String[] recipe = new String[] {
        "aggregate-stats :data_transfer :duration total_size_mb total_time_sec 'MB' 's';"
      };

      RecipeParser parser = TestingRig.parse(recipe);
      List<Directive> directives = parser.parse();
      Assert.assertEquals(1, directives.size());
      Assert.assertEquals("AggregateStats", directives.get(0).getClass().getSimpleName());
    }
}